                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <input id="namasatuan" type="text" class="form-control form-control-sm shadow" placeholder="Nama" style="height: 28px;">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <form class="form form-vertical">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-sm btn-info btn-shadowe" onclick="update()">Update</button>
                        <button type="button" class="btn btn-sm btn-outline-danger" data-dismiss="modal">Batal</button>
                    </div>
                </form>
            </div>
...
function simpan(){
        ...
        success: function (data)
        {
            if (data.success) {
                ...
                $('#form')[0].reset();
                tampilPesan('SUCCESS','Berhasil Menambah Data Satuan','success','center')
                $('#insert').modal('hide');
            } else {
...
function update(){
    var id = $('#id_data').val();
    $.ajax({
        url: '/newposmp1/satuan/update/' + id,
        type: "PATCH",
        data: {
            id: id,
            nama: $("#namasatuanedit").val(),
            _token: "{{ csrf_token() }}"
        },
        success: function (data) {
            if (data.success == 200) {
            $('#example').DataTable().ajax.reload(null, false);
            $('#edit').modal('hide');
            tampilPesan('SUCCESS','Berhasil Update Satuan','success','center')
            }
            $('#edit').modal('show');
...
function hapus(id){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
            url: `/newposmp1/delete-satuan/${id}`,
            method: "DELETE",
            data: {
            "_token": "{{ csrf_token() }}",
            },
