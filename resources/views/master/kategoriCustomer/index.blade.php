        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">TAMBAH KATEGORI CUSTOMER</h5>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <input id="namakategoricustomer" type="text" class="form-control form-control-sm shadow" placeholder="Nama" style="height: 28px;">
                                </div>
                            </div>
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label for="">Diskon (%)</label>
                                    <input id="diskonkategoricustomer" type="number" class="form-control form-control-sm shadow" placeholder="10" style="height: 28px;">
                                </div>
                            </div>
                        </div>
                    </div>
...
<div id="edit" class="modal fade" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-dark">
                <h5 class="modal-title text-white">EDIT KATEGORI CUSTOMER</h5>
                <button type="button" class="close text-white" data-dismiss="modal">×</button>
            </div>
...
function simpan(){
    $.ajax({
    url: "save-kategori-customer",
    type: "POST",
    data: {
    nama: $("#namakategoricustomer").val(),
    diskon: $("#diskonkategoricustomer").val(),
    _token: "{{ csrf_token() }}"
    },
    success: function (data)
    {
...
function update(){
    var id = $('#id_data').val();
    $.ajax({
        url: '/kategori-customer/update/' + id,
        type: "PATCH",
        data: {
            id: id,
            nama: $("#namakategoricustomeredit").val(),
            diskon: $("#diskonkategoricustomeredit").val(),
            _token: "{{ csrf_token() }}"
        },
        success: function (data) {
            if (data.success == 200) {
...
function hapus(id){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
            url: `/delete-kategori-customer/${id}`,
            method: "DELETE",
            data: {
            "_token": "{{ csrf_token() }}",
            },
